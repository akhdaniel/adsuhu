...
"""
    ...
    def generate_output_html(self):
        try:
            ...
            _logger.warning("JSON parse failed for vit.angle_hook(%s), retrying with fix_json: %s", self.id, e)
            try:
                ...
                _logger.error("vit.angle_hook(%s) fallback also failed: %s", self.id, e2)
